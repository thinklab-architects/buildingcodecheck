...
const ResultCard = ({ title, result }) => {
    ...
    return (
        <div className={`h-full p-4 rounded-xl border transition-all ${ok ? 'bg-emerald-50/50 border-emerald-100' : 'bg-rose-50/50 border-rose-100'
            ...
            {computedPath && (
                <div className="w-full bg-slate-50 p-2 rounded text-sm text-slate-600 mt-1 mb-1 border border-slate-200">
...
const SectionLayout = ({ children, result, title, extraResult }) => (
    ...
    return (
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col gap-2">
                ...
                {/* 1. 基本資料 */}
                <SectionLayout result={results.checks.govApplicants} title="檢核結果：機關申購條件">
                ...
                {/* 2. 擬合併土地與建築條件 */}
                <SectionLayout result={results.checks.nonSaleProhibitions} title="檢核結果：不得讓售情形">
                    <GlassCard className="h-full">
                ...
                {/* 3. 協議調整地形 */}
                <SectionLayout result={results.checks.landAdjustment} title="檢核結果：協議調整地形">
                    <GlassCard className="h-full">
                ...
                {/* 4. 占用與補償 */}
                <SectionLayout result={results.checks.occupancy} title="檢核結果：占用與補償">
                    <GlassCard className="h-full">
                ...
                {/* 5. 土地權利與共有 */}
                <SectionLayout
                    result={results.checks.priorityRightHolders}
                    title="檢核結果：優先購買權"
                    extraResult={<ResultCard title="檢核結果：共有土地處理" result={results.checks.coOwnership} />}
                >
                    <GlassCard className="h-full">
                ...
                {/* 6. 價格與承諾 & 7. 擬採處理方式 */}
                <SectionLayout result={results.checks.salePath} title="檢核結果：法定處理路徑">
                ...
                {/* 8. 應附書件 */}
                <SectionLayout result={results.checks.applicationDocuments} title="檢核結果：應附書件">
                    <GlassCard className="h-full">
